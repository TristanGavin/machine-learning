...
class LinearLayer:
  ...
  def backward(self, grad):
    self.grad_weights = np.matmul(np.transpose(self.input),grad)   # x^t*grad (input_dim x output_dim) 
    self.grad_bias = grad.sum()                                    # sum_dL/dz (1 x outputdim)
    return np.matmul(grad,np.transpose(self.weights))              # (n x inputdim)
